...
class deeplab_xception_transfer_basemodel_synBN(deeplab_xception_synBN.DeepLabv3_plus):
    ...
    def forward(self, input,adj1_target=None, adj2_source=None,adj3_transfer=None ):
        x, low_level_features = self.xception_features(input)
        print( "[deeplab_xception_transfer_basemodel_synBN / after encoder] x.shape", x.shape )                                     # 
        print( "[deeplab_xception_transfer_basemodel_synBN / after encoder] low_level_features.shape", low_level_features.shape )   #
